/**
 * MODULE: Coder Node - Batch Code Generation with Multi-Tier Fallback
 *
 * This module generates code files based on a plan from the planner node.
 * For large plans (> 8 tasks), it uses batch processing to work within
 * LLM token limits while maximizing throughput.
 *
 *
 * STRATEGY SELECTION:
 * ┌─────────────────────────────────────────────────────────────┐
 * │  Input: Plan with N tasks                                   │
 * └─────────────────────┬───────────────────────────────────────┘
 *                       │
 *         ┌─────────────▼─────────────┐
 *         │ N ≤ 8?                    │
 *         │ YES → Single-pass LLM    │
 *         │ NO  → Split into batches │
 *         └─────────────┬─────────────┘
 *
 *
 * THREE-TIER RETRY PATTERN:
 * When generating code, failures are handled at multiple levels:
 *
 *   Tier 1: Batch Retry (processBatchWithRetry)
 *   ┌──────────────────────────────────────────┐
 *   │ Try batch of 8 files → Failed?          │
 *   │ Wait (exponential backoff) → Retry      │
 *   │ Max 3 attempts per batch                │
 *   └──────────────────────────────────────────┘
 *           │
 *           ▼ (if all 3 attempts fail)
 *   Tier 2: Single-File Fallback (processSingleFile)
 *   ┌──────────────────────────────────────────┐
 *   │ Process each file individually          │
 *   │ Each file gets its own 3 retries        │
 *   │ Slower but more reliable                │
 *   └──────────────────────────────────────────┘
 *           │
 *           ▼ (if single-file also fails)
 *   Tier 3: Report & Continue
 *   ┌──────────────────────────────────────────┐
 *   │ Add to failedTasks list                 │
 *   │ Continue with remaining files           │
 *   │ Reviewer node may trigger codegen retry │
 *   └──────────────────────────────────────────┘
 *
 *
 * PROGRESSIVE FILE WRITING:
 * Files are written to sandbox IMMEDIATELY after each batch completes,
 * not after all batches finish. This enables:
 * - Live preview updates in the frontend
 * - Dev server hot-reload during generation
 * - Reduced memory usage (no accumulating all files)
 *
 *
 * COMMAND DEDUPLICATION:
 * Different batches may request the same package install. The `deduplicateCommands`
 * function ensures each command runs only once (case-insensitive comparison).
 */

import { HumanMessage, SystemMessage } from "@langchain/core/messages";
import {
  INITIAL_SYSTEM_PROMPT,
  CODER_WITH_PLAN_PROMPT,
} from "../systemPrompts";
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const structuredModel =
        createLLM().withStructuredOutput(CodeGenerationSchema);
...
async function processSingleFile(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      log.codegen(
        `    Single file attempt ${attempt} failed:`,
        error instanceof Error ? error.message : error
      );
      if (attempt < MAX_RETRIES) {
...
/**
 * Main coder node - decides between batch and single-pass generation.
 *
 * Decision logic:
 * - Plan has ≤ 8 tasks (or no plan) → Use singlePassGeneration
 * - Plan has > 8 tasks → Split into batches, process with progressive writing
 *
 * The `writeFiles` callback from config.configurable enables progressive
 * writing - files appear in sandbox immediately after each batch/generation.
 */
export async function coderNode(
  ...
  log.codegen(
    "Prompt:",
    state.prompt.slice(0, 100) + (state.prompt.length > 100 ? "..." : "")
  );
  ...
  if (!state.plan || state.plan.tasks.length <= BATCH_SIZE) {
    log.codegen(
      "Using single-pass generation (plan has ≤" + BATCH_SIZE + " tasks)"
    );
  ...
  log.codegen(
    `Executing plan with ${plan.tasks.length} tasks using batch processing:`
  );
  plan.tasks.forEach((t) =>
    log.codegen(`  - ${t.action} ${t.file}: ${t.description}`)
  );

  const batches = createBatches(plan.tasks);
  log.codegen(
    `Split into ${batches.length} batches of up to ${BATCH_SIZE} files each`
  );
  ...
  for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
    ...
    if (batchResult) {
      ...
      log.codegen(
        `Batch ${batchNumber} generated ${batchResult.files.length} files`
      );
      batchResult.files.forEach((f) => log.codegen(`  ✓ ${f.filePath}`));
    } else {
      // Batch failed after all retries - try processing files individually
      log.codegen(
        `Batch ${batchNumber} failed after ${MAX_RETRIES} retries. Falling back to single-file processing...`
      );
    ...
    if (batchFiles.length > 0) {
      ...
      if (writeFiles) {
        log.codegen(
          `Writing ${batchFiles.length} files from batch ${batchNumber}...`
        );
        try {
  ...
  const missingTasks = plan.tasks.filter(
    (t) => t.action !== "delete" && !generatedPaths.has(t.file)
  );
  ...
  if (failedTasks.length > 0) {
    log.codegen(
      `WARNING: ${failedTasks.length} files failed to generate after all retries`
    );
  }
  ...
  log.codegen(
    `Generated ${allFiles.length} files and ${uniqueCommands.length} commands in ${duration}s across ${batches.length} batches`
  );
  if (writeFiles) {
    log.codegen(
      `Progressive writing: ${allWrittenFiles.length} files written during generation`
    );
  }
...
async function singlePassGeneration(
  ...
  if (state.plan) {
    ...
    const missingTasks = state.plan.tasks.filter(
      (t) => t.action !== "delete" && !generatedPaths.has(t.file)
    );
  ...
  const duration = ((Date.now() - startTime) / 1000).toFixed(1);
  log.codegen(
    `Generated ${result.files.length} files and ${result.commands?.length || 0} commands in ${duration}s`
  );
