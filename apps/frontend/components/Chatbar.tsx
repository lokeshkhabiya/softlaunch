...
export default function ChatBar({
  ...
  // Load initial messages when they change (e.g., when loading saved project)
  useEffect(() => {
    if (
      initialMessages &&
        ...
        {messages.map((message, index) =>
          message.type === "user" ? (
            ...
            <div key={index} className="flex justify-start">
              <div className="text-gray-300 px-1 py-2 max-w-[80%]">
                {message.toolCalls && message.toolCalls.length > 0 && (
                  <div className="mb-2 text-xs text-gray-500">
                    ...
                    {message.toolCalls.map((tc, i) => (
                      <span
                        key={i}
                        className="inline-block bg-gray-700 rounded px-2 py-0.5 mr-1"
                      >
                        {tc.name}
                      </span>
                    ))}
              ...
              {toolCalls.length > 0 && (
                <div className="mb-2 text-xs text-gray-500">
                  ...
                  {toolCalls.map((tc, i) => (
                    <span
                      key={i}
                      className="inline-block bg-blue-700/50 rounded px-2 py-0.5 mr-1 animate-pulse"
                    >
                      {tc.name}
                    </span>
                  ))}
