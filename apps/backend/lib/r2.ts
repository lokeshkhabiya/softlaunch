...
export async function getProjectCodeHash(
  ...
  try {
    ...
    // Use -path for directories and -name for files
    const dirExcludes = [
      "node_modules",
      ".git",
      "dist",
    ...
    // Get a hash based on file names and sizes only (NOT modification time)
    // Modification time can differ after restore from backup even if content is same
    ...
    // Fallback: simpler hash based on file paths and sizes
