...
import { activeSandboxes, BACKUP_DEBOUNCE_MS } from "@/routes/session";
...
export async function handleRefresh(req: Request, res: Response) {
  ...
  if (wasPending) {
    console.log(
      `[REFRESH] User returned, cancelled pending shutdown for ${sandboxId}`
    );
  }
...
export async function handleNotifyLeaving(req: Request, res: Response) {
  ...
  if (!projectId || !userId) {
    return res
      .status(400)
      .json({ error: "Session missing projectId or userId" });
  }
  ...
  res.json({
    success: true,
    message: `Sandbox shutdown scheduled in ${BACKUP_DEBOUNCE_MS / 1000} seconds`,
    scheduledShutdownAt: new Date(
      Date.now() + BACKUP_DEBOUNCE_MS
    ).toISOString(),
  });
}
...
export async function handleGetStatus(req: Request, res: Response) {
  ...
  for (const [sandboxId, session] of activeSandboxes) {
    if (session.projectId === projectId) {
      // Only show backing_up when actually performing backup, not during debounce
...
export async function handleDelete(req: Request, res: Response) {
  ...
  try {
    if (session.userId && session.projectId && isR2Configured()) {
      ...
      const codeChanged =
        !currentHash ||
        !session.lastCodeHash ||
        currentHash !== session.lastCodeHash;

      if (codeChanged) {
        console.log(
          `[DELETE] Code changed, preparing to backup project ${session.projectId}`
        );
        ...
        if (!mountReady) {
          ...
          console.log(
            `[DELETE] Backing up project ${session.projectId} before manual termination`
          );
          const backed = await backupProject(
            session.sandbox,
            session.userId,
            session.projectId
          );
          if (backed) {
            ...
            console.log(
              `[DELETE] ✓ Project ${session.projectId} backed up successfully`
            );
          } else {
            console.error(
              `[DELETE] ✗ Failed to backup project ${session.projectId}`
            );
          }
        }
      } else {
        console.log(
          `[DELETE] Code unchanged since last backup, skipping backup for ${session.projectId}`
        );
      }
    }
